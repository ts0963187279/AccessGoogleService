...
public class BuildAuthorizeUrl implements Mission<Void> {
    ...
    public BuildAuthorizeUrl(OAuth2Data oAuth2Data){
    ...
    @Override
    public String execute(Void noArguments){
        GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(transport,jsonFactory,clientId,clientSecrets,scopes)
                .setAccessType("offline").setApprovalPrompt("auto").build();
        GoogleAuthorizationCodeRequestUrl url = flow.newAuthorizationUrl();
        url.setRedirectUri(redirectUri);
        url.setApprovalPrompt("auto");
